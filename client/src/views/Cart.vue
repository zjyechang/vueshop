<template>
  ...
  <defs>
	...
	<symbol id="icon-clock" viewBox="0 0 32 32">
	  ...
	  <nav-header/>
	  <nav-bread><span>购物车</span></nav-bread>
		<div class="container">
		<div class="cart">
						...
						<li v-for="(item,index) in cartList" :key="index">
							<div class="cart-tab-1">
								<div class="cart-item-check">
									<a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='1'}" @click="cartUpdate('checked',item)">
								...
								<div class="item-quantity">
									<div class="select-self select-self-open">
										<div class="select-self-area">
											<a class="input-sub" @click="cartUpdate('min',item)">-</a>
											<span class="select-ipt">{{item.productNum}}</span>
											<a class="input-add" @click="cartUpdate('add',item)">+</a>
										</div>
									</div>
								</div>
							</div>
...
<script>
	...
	export default {
		...
		computed:{
			...
			checkedAllFlag(){
				// 当前选中商品和购物车所有商品进行比较
				return this.checkoutCount == this.cartList.length;
			},
			totalPrice(){
				let money = 0;
				this.cartList.forEach( (item)=>{
		...
		methods:{
			...
			toggleCheckAll(){
				...
				this.$http.post('/users/checkAll',{checkAll:this.checked}).then( res=>{
					console.log(res);
				})
			},
			delCartConfirm(item){
